...
def train(args, dataset, generator, discriminator):
    ...
    resolution = 4 * 2 ** step
    data_loader = sunnerData.DataLoader(dataset, batch_size=args.batch.get(resolution, args.batch_default), resolution=resolution, shuffle=True)
    ...
    for i, (real_img,) in enumerate(bar):
        ...
        if (i + 1) % 10000 == 0:
            ...
            utils.save_image(torch.cat(images, 0), f"sample/{str(i + 1).zfill(6)}.png", nrow=gen_i, normalize=True,
                             range=(-1, 1))
            torch.save(g_running.state_dict(), f"checkpoint/{str(i + 1).zfill(6)}.model")

        state_msg = f"Size: {4 * 2 ** step}; G: {gen_loss_val:.3f}; D: {disc_loss_val:.3f};" f" Grad: {grad_loss_val:.3f}; Alpha: {alpha:.5f}"
...
if __name__ == "__main__":
    ...
    dataset = sunnerData.ImageDataset(
        ...
        transform=transforms.Compose(
            [sunnertransforms.Resize((128, 128)), sunnertransforms.ToTensor(), sunnertransforms.ToFloat(), sunnertransforms.Transpose(sunnertransforms.BHWC2BCHW), sunnertransforms.Normalize()]
        ),
    )
